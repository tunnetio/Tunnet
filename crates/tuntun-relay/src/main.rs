...
#[derive(Parser, Debug)]
#[command(name = "tuntun-relay", about = "TunTun public edge relay")]
struct Cli {
...
struct RunArgs {
    ...
    token: Option<String>,
    #[arg(long, env = "TUNTUN_RELAY_HTTPS_BIND", default_value = "0.0.0.0:443")]
    https_bind: String,
    #[arg(long, env = "TUNTUN_RELAY_CERT")]
    cert: Option<PathBuf>,
    ...
    #[arg(long, env = "TUNTUN_RELAY_OPEN_AUTH")]
    open_auth: bool,
    ...
    #[arg(long, env = "TUNTUN_RELAY_ACME_STAGING")]
    acme_staging: bool,
}
...
fn init_logging(json: bool) {
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info,tuntun_relay=debug"));
...
async fn run(cfg: RunConfig) -> anyhow::Result<()> {
    let state_dir = cfg.state_dir.clone().unwrap_or_else(|| {
        ...
        PathBuf::from(base).join("tuntun-relay")
    });
