//! `tunnet login` / `tunnet logout` - OAuth device authorization (RFC 8628).

use anyhow::{Context, bail};
use clap::Args;
use tunnet_core::{CliAuthTokens, StatePaths};
...
async fn poll_for_token(
    ...
    loop {
        if std::time::Instant::now() >= deadline {
            bail!("device code expired - run `tunnet login` again");
        }
        ...
        match tokens.error.as_deref() {
            ...
            Some("expired_token") => bail!("device code expired - run `tunnet login` again"),
            Some(other) => bail!(
                "device token error: {} ({})",
