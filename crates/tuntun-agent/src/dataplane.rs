...
use tuntun_common::{DeviceProfile, DnsConfig, TUNNEL_ALPN};
use tuntun_core::ipc::dataplane::DataPlaneCmdRx;
use tuntun_core::ipc::{DataPlaneHandle, recv_cmd};
use tuntun_core::{AclEngine, ConnPool, CoreNode, RoutingTable};
use uuid::Uuid;
...
fn route_snapshot(
    ...
    if let Some(snap) = tuntun_core::state::load_snapshot_cache(&node.paths)
